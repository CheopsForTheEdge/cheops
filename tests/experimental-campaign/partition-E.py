...
print(roles["faulty"][0].alias)


netem = en.NetemHTB()
netem.add_constraints(src=roles["faulty"],
                      dest=roles["cheops"],
                      symmetric=True,
                      loss="100%",
                      delay="20ms",
...
with en.actions(roles=roles["cheops"], gather_facts=False) as p:
    for node in roles["cheops"]:
        p.shell(
            task_name="Pings while cut",
            cmd=f"ping -c 5 {node.address} >> /tmp/results/ping_while_cut_$(date +'%Y-%m-%d_%H-%M-%S').log",
            ignore_errors=True
        )
...
with en.actions(roles=roles["cheops"], gather_facts=True) as p:
    p.shell(
        task_name="Get pods",
        cmd="kubectl get pods > /tmp/results/get_pods_after_update_main_$(date +'%Y-%m-%d_%H-%M-%S').log"
    )
    p.shell(
        task_name="Describe pod",
        cmd="kubectl describe pod simpleapp-pod > /tmp/results/describe_pods_after_update_main__$(date +'%Y-%m-%d_%H-%M-%S').log",
        ignore_errors=True
    )

with en.actions(roles=roles["faulty"][0], gather_facts=False) as p:
    p.lineinfile(
        task_name="Edit simple-pod.yml to update",
        search_string="    app.kubernetes.io/name: SimpleApp",
        line="    app.kubernetes.io/name: UpdatedSimpleAppFaulty",
        path=f"{cheops_location}/simple-pod.yml"
    )
    p.shell(
        task_name="Apply pod config",
        cmd=f"kubectl --kubeconfig ~/.kube/config.proxified apply -f {cheops_location}/simple-pod.yml --server-side=true > /tmp/results/update_faulty_pod_$(date +'%Y-%m-%d_%H-%M-%S').log",
        ignore_errors=True
    )
    results = p.results
...
with en.actions(roles=roles["cheops"], gather_facts=True) as p:
    p.shell(
        task_name="Get pods",
        cmd="kubectl get pods > /tmp/results/get_pods_after_update_faulty_$(date +'%Y-%m-%d_%H-%M-%S').log"
    )
    p.shell(
        task_name="Describe pod",
        cmd="kubectl describe pod simpleapp-pod > /tmp/results/describe_pods_after_update_faulty_$(date +'%Y-%m-%d_%H-%M-%S').log",
        ignore_errors=True
    )
